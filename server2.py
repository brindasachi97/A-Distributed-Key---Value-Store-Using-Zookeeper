    ...
    def run(self): 
        ...
        while True : 
            ...
            if choice==1:
            	...
            	insert(key,value,"server2",replicate)
            	modify_mappings()
            ...
            if choice==2:
            	client_key=d[1]
            	MESSAGE=str(select(client_key,"server2"))
            	conn.send(MESSAGE.encode('utf-8'))
...
def starting():
    ...
    if(check_master==[]):
        ...
        replicate("server2")
        keys_in_server=select_all_keys("server2") #list
        d={}
        d[connected_port]=keys_in_server
        d=collections.OrderedDict(d)
        register_server(d,"server2")
        time.sleep(10)
        #put_watch()
